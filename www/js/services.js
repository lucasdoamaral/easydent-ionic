...
angular.module('easydent.services', [])
	...
	.service('UsuarioService', function ($http, SERVER) {
		...
		return {
			...
			buscar: function (usuarioId) {
				return $http.get(SERVER.url + '/usuarios/' + usuarioId);
			},
			buscarUsuarioLogado: function () {
				return $http.get(SERVER.url + '/usuarios/logado');
			},
			alterarSenha: function (usuario) {
				return $http.post(SERVER.url + '/usuarios/alterarsenha', usuario);
			},
			salvar: function (usuario) {
				return $http.post(SERVER.url + '/usuarios', usuario);				
			},
			checkUsername: function (nomeUsuario) {
				return $http.get(SERVER.url + '/usuarios/checkusername/' + nomeUsuario);
			},
			checkEmail: function (email) {
				return $http.get(SERVER.url + '/usuarios/checkemail/' + email);
			}
		};
